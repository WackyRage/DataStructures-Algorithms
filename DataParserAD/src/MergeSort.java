...
public class MergeSort{
    static void merge(Application application, int p, int q, int r){
        if(application.getSelectedDataStructure() == Application.DataStructures.LinkedList) {
            ...
            for (int i = 0; i < n1; i++) {
                L.add(i, application.getArrayListGame().get(p + i));
            }
            for (int j = 0; j < n2; j++) {
                M.add(j, application.getArrayListGame().get(q + 1 + j));
            }
            ...
            while (i < n1 && j < n2) {
                if (L.get(i).getName().compareTo(M.get(j).getName()) <= 0) {
                    application.getArrayListGame().set(k, L.get(i));
                    i++;
                } else {
                    application.getArrayListGame().set(k, M.get(j));
                    j++;
                }
                k++;
            }
